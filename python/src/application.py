...
class Application:
    ...
    def run(self) -> None:
        ...
        if len(self.files) > 1:
            self.__output__(
                f'========== [{
                    self.__exec_mode__()}] Total File Count to Clean: {
                    len(
                        self.files)} ==========')
            self.__output__(
                f'========== [{
                    self.__exec_mode__()}] Start Cleaning {
                    self.pattern} ==========')
            for file in self.files:
                self.__output__(
                    f'========== [{self.__exec_mode__()}] Cleaning {file} ==========')
                if self.mode == 'e':
                    os.remove(file)
            self.__output__(
                f'========== [{
                    self.__exec_mode__()}] Cleaned {
                    self.pattern} ==========')
            self.__output__(
                f'========== [{
                    self.__exec_mode__()}] Total Cleaned File Count: {
                    len(
                        self.files)} ==========')
        else:
            self.__output__(
                f'========== [{
                    self.__exec_mode__()}] No {
                    self.pattern} Remains ==========')
